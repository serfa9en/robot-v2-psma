...
export function getDisease (ind, val) {
  ...
  switch (ind) {
    case 0: {
      ...
      str += '<ul><li>Рекомендации по здоровому образу жизни</li>'
      str += '<li>Правила ухода за ногами при сахарном диабете</li></ul></p>'
      break
    }
    case 1: {
    ...
    case 2: {
      if (val <= 5) str += ' - низкий риск развития'
    ...
    case 3: {
      if (val <= 4) str += 'Отсутствие или минимальный уровень депрессии<br>'
        ...
        str += '<ul><li>отказаться от употребления табака;</li>'
        str += '<li>поддерживайте нормальную массу тела (индекс массы тела  от 18.5 до 25);</li>'
        str += '<li>здоровый рацион питания, включающий фрукты и овощи;</li>'
        str += '<li>регулярная физическая активность;</li>'
        str += '<li>прекращение или ограничение употребления алкоголя;</li>'
        str += '<li>исключение ультрафиолетового облучения (прежде всего, в результате воздействия солнечных лучей или аппаратов для искусственного загара) и/или использование средств защиты от солнечных лучей;</li>'
        str += '<li>снижение воздействия таких факторов, как загрязнение атмосферного воздуха и воздуха внутри помещений.</li></ul>'
      }
      if (val >= 5 && val <= 14) str += 'Вам необходимо обратиться к профильному специалисту (психологу или психотерапевту)'
      if (val >= 15 && val <= 19) str += 'Вам необходимо обратиться к профильному специалисту (психологу или психотерапевту), потребуется назначение комплексной схемы лечения'
      if (val < 20) {
        str += '<ul><li>Соблюдайте режим сна и бодрствования</li>'
        str += '<li>Откажитесь от кофе и алкоголя, особенно в вечернее время</li>'
        str += '<li>Рекомендуется вести активный образ жизни: гулять не менее получаса ежедневно, выполнять легкую гимнастику</li>'
        str += '<li>Возможно применение массажа, гидротерапии и лечебной физкультуры</li>'
        str += '<li>Особое внимание следует уделить обучению методикам снятия стресса, созданию благоприятной атмосферы на работе и дома</li></ul>'
      }
      break
    }
    case 4: {
